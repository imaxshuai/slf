...
class CarServerList extends Component{
    ...
    componentDidMount(){
        ...
        this.props.carServerActions.getCarServerList(filters, {data: []});
    }

    componentWillUnmount(){
        this.props.carServerActions.getCarServerList('no', {data: []});

    }

    getCarServerListByFilter = (data)=>{
        ...
        if(typeof(data[1])=='string'){
            ...
            if(data[1]==null){
                ...
                }else if(data[0]=='carServer_size'){
                    filters.max_carServer_size = null;
                    filters.min_carServer_size = null;
                }
            }else{
    ...
    //获取下拉加载更多数据
    _getMoreCarServer = ()=>{
        if(this.props.carServerList.data.length>=10&&!this.props.carServerList.isEnd){
            this.props.carServerActions.getCarServerList(this.props.filter, this.props.carServerList);
        }
    };
    ...
    _footer =()=>{
        return (
            <View>
                {this.props.carServerList.isEnd
                    ?
        ...
        return (
            ...
            <View style={styles.container}>
                ...
                <AreaModel
                    ...
                    chooseArea={(area)=>this.getCarServerListByFilter(area)}
                ...
                <TypeModel
                    ...
                    chooseType={(price)=>this.getCarServerListByFilter(price)}
                ...
                {params.filter[1]!=null?
                    (
                        <TypeModel
                            ...
                            chooseType={(carServer_type)=>this.getCarServerListByFilter(carServer_type)}
                ...
                {params.filter[2]!=null?(
                    params.filter[2].length>1?(
                        <OtherModel
                            ...
                            chooseFilter={(data)=>this.getCarServerListByFilter(data)}
                        ...
                        <TypeModel
                            ...
                            chooseType={(other)=>this.getCarServerListByFilter(other)}
                            bgClickHideModel={()=>this.setState({showTypeModel: false})}
                        />
                    )
                ): null}
                <Spinner
                    ...
                    isVisible={(this.props.carServerList.data.length<=0)&&(this.props.carServerList.isEnd==false)}
                    size={50}
                    type='9CubeGrid'
                    color='red'
                />
                ...
                <FlatList
                    ...
                    data={this.props.carServerList['data']}
                    keyExtractor={(item)=>item.id}
                    initialNumToRender={5}
                    onEndReachedThreshold={0.3}
                    onEndReached={this._getMoreCarServer.bind(this)}
...
const mapStateToProps = (state)=>{
    return {
        nav: state.nav,
        carServerList: state.carServerList,
        filter: state.filter
    }
};
...
const mapDispatchToProps = (dispatch)=>{
    return {
        ...
        carServerActions: bindActionCreators(carServerActions, dispatch),
