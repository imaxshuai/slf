...
import * as recoveryActions from '../../../../redux/actions/recovery';
...
class RecoveryList extends Component{
    ...
    componentDidMount(){
        ...
        this.props.recoveryActions.getRecoveryList(filters, {data: []});
    }

    componentWillUnmount(){
        this.props.recoveryActions.getRecoveryList('no', {data: []});

    }

    getRecoveryListByFilter = (data)=>{
        ...
        if(typeof(data[1])=='string'){
            ...
            if(data[1]==null){
                ...
                }else if(data[0]=='recovery_size'){
                    filters.max_recovery_size = null;
                    filters.min_recovery_size = null;
                }
            }else{
        ...
        return (
            ...
            <View style={styles.container}>
                ...
                <AreaModel
                    ...
                    chooseArea={(area)=>this.getRecoveryListByFilter(area)}
                ...
                <TypeModel
                    ...
                    chooseType={(price)=>this.getRecoveryListByFilter(price)}
                ...
                {params.filter[1]!=null?
                    (
                        <TypeModel
                            ...
                            chooseType={(recovery_type)=>this.getRecoveryListByFilter(recovery_type)}
                ...
                {params.filter[2]!=null?(
                    params.filter[2].length>1?(
                        <OtherModel
                            ...
                            chooseFilter={(data)=>this.getRecoveryListByFilter(data)}
                        ...
                        <TypeModel
                            ...
                            chooseType={(other)=>this.getRecoveryListByFilter(other)}
                            bgClickHideModel={()=>this.setState({showTypeModel: false})}
                        />
                    )
                ): null}
                <Spinner
                    ...
                    isVisible={(this.props.recoveryList.data.length<=0)&&(this.props.recoveryList.isEnd==false)}
                    size={50}
                    type='9CubeGrid'
                    color='red'
                />
                ...
                <FlatList
                    ...
                    data={this.props.recoveryList['data']}
                    keyExtractor={(item)=>item.id}
                    initialNumToRender={5}
                    onEndReachedThreshold={0.3}
                    onEndReached={this._getMoreRecovery.bind(this)}
...
const mapStateToProps = (state)=>{
    return {
        nav: state.nav,
        recoveryList: state.recoveryList,
        filter: state.filter
    }
};
