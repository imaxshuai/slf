...
import * as esActions from '../../../../redux/actions/es';
...
class EsList extends Component{
    ...
    componentDidMount(){
        ...
        this.props.esActions.getEsList(filters, {data: []});
    }

    componentWillUnmount(){
        this.props.esActions.getEsList('no', {data: []});

    }

    getEsListByFilter = (data)=>{
        ...
        if(typeof(data[1])=='string'){
            ...
            if(data[1]==null){
                ...
                }else if(data[0]=='es_size'){
                    filters.max_es_size = null;
                    filters.min_es_size = null;
                }
            }else{
    ...
    _getMoreEs = ()=>{
        console.log('无线加载');
        if(this.props.esList.data.length>=10&&!this.props.esList.isEnd){
            this.props.esActions.getEsList(this.props.filter, this.props.esList);
        }
    };
        ...
        return (
            ...
            <View style={styles.container}>
                ...
                <AreaModel
                    ...
                    chooseArea={(area)=>this.getEsListByFilter(area)}
                ...
                <TypeModel
                    ...
                    chooseType={(price)=>this.getEsListByFilter(price)}
                ...
                {params.filter[1]!=null?
                    (
                        <TypeModel
                            ...
                            chooseType={(es_type)=>this.getEsListByFilter(es_type)}
                ...
                {params.filter[2]!=null?(
                        params.filter[2].length>1?(
                            <OtherModel
                                ...
                                chooseFilter={(data)=>this.getEsListByFilter(data)}
                            ...
                            <TypeModel
                                ...
                                chooseType={(other)=>this.getEsListByFilter(other)}
                                bgClickHideModel={()=>this.setState({showTypeModel: false})}
                            />
                        )
                    ): null}
                <Spinner
                    ...
                    isVisible={(this.props.esList.data.length<=0)&&(this.props.esList.isEnd==false)}
                    size={50}
                    type='9CubeGrid'
                    color='red'
                />
                ...
                <FlatList
                    ...
                    renderItem={({item})=>(
                        <ListItemHouseComponent
                            ...
                            unit={params.unit}
                        />
                    )}                    data={this.props.esList['data']}
                    keyExtractor={(item)=>item.id}
                    initialNumToRender={5}
                    onEndReachedThreshold={0.3}
                    onEndReached={this._getMoreEs.bind(this)}
...
const mapDispatchToProps = (dispatch)=>{
    return {
        esActions: bindActionCreators(esActions, dispatch)
    }
};
