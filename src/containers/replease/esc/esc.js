...
import {CarColor, CardYear, EsPhoneBrand, Jiaoqiangxian,Shangyexian} from "../../../components/picker";
...
export class Replease5to1 extends Component {
    ...
    constructor(...props){
        ...
        this.state = {
            ...
            showLoad: false,
            images: null,                   //图片
            transmission: null,             //变速器
            describe: null,                 //描述
            accident: null,                 //有无事故
            brand: null,                    //牌子
            color: null,                    //颜色
            card_year: null,                //上牌时间
            jiaoqiangxian: null,            //交强险
            shangyexian: null,              //交强险
        }
    }
    ...
    doSubmit = ()=>{
        ...
        let repleaseInfo = {
            ...
            mileage: this.refs.mileage._lastNativeText,
            ...
            displacement: this.refs.displacement._lastNativeText,


            //picker选取的值
            area: this.state.area==null?undefined:this.state.area.join(''),
            brand: this.state.brand==null?undefined:this.state.brand.join(''),
            color: this.state.color==null?undefined:this.state.color.join(''),
            card_year: this.state.card_year==null?undefined:this.state.card_year.join(''),
            jiaoqiangxian: this.state.jiaoqiangxian==null?undefined:this.state.jiaoqiangxian.join(''),
            shangyexian: this.state.shangyexian==null?undefined:this.state.shangyexian.join(''),
        ...
        (repleaseInfo.mileage==null)||(repleaseInfo.mileage=='')?message.push('请填写行驶里程!'):null;
        (repleaseInfo.displacement==null)||(repleaseInfo.displacement=='')?message.push('请填写排量!'):null;
        ...
        repleaseInfo.brand==null?message.push('请选择品牌!'):null;
        repleaseInfo.color==null?message.push('请选择颜色!'):null;
        repleaseInfo.card_year==null?message.push('请选择上牌年份!'):null;
        repleaseInfo.jiaoqiangxian==null?message.push('请选择交强险到期时间!'):null;
        repleaseInfo.shangyexian==null?message.push('请选择商业险到期时间!'):null;
        ...
        if(message.length>0){
            ...
            Http.post(Ip+'api/esc/create', repleaseInfo ,{'Content-Type': 'application/json'},)
                .then( res=>{
                    if(res.success){
                        this.setState({
                            showLoad: false
                ...
                <KeyboardAwareScrollView>
                    ...
                    <ScrollView>
                        ...
                        <View style={styles.item}>
                            ...
                            <View style={styles.info}>
                                <Text style={styles.infoTitle}>行驶里程(万公里)</Text>
                                <TextInput
                                    autoCapitalize='none'
                                    placeholder="请填写行驶里程"
                                    placeholderTextColor="#ccc"
                            ...
                            <View style={styles.info}>
                                <Text style={styles.infoTitle}>排量(升)</Text>
                                <TextInput
                                    autoCapitalize='none'
                                    placeholder="请填写排量"
                                    placeholderTextColor="#ccc"
                            ...
                            <View style={styles.pickerGroup}>
                                <EsPhoneBrand
                                    brand={(data)=>this.setState({brand: data})}
                                    data={this.props.navigation.state.params[1]}
                                />
                                <CarColor
                                    color={(data)=>this.setState({color: data})}
                                />
                            </View>
                            <View style={styles.pickerGroup}>
                                <CardYear
                                    card_year={(data)=>this.setState({card_year: data})}
                                />
                                <Jiaoqiangxian
                                    jiaoqiangxian={(data)=>this.setState({jiaoqiangxian: data})}
                                />
                                <Shangyexian
                                    shangyexian={(data)=>this.setState({shangyexian: data})}
                                />
                            </View>
                        ...
                        <View style={styles.item}>
                            ...
                            <View style={styles.info}>
                                <Text style={styles.infoTitle}>价格(万元)</Text>
                                <TextInput
                                    autoCapitalize='none'
                                    placeholder="请填写价格"
                                    placeholderTextColor="#ccc"
                        ...
                        <View style={styles.item}>
                            ...
                            <View style={styles.info}>
                                <View style={styles.info}>
                                    <Text style={styles.infoTitle}>变速器</Text>
                                    <Radio data={['手动挡','自动挡']} select={(data)=>this.setState({transmission: data})}/>
                                </View>
                            </View>

                            <View style={styles.info}>
                                <View style={styles.info}>
                                    <Text style={styles.infoTitle}>有无事故</Text>
                                    <Radio data={['无事故','有事故']} select={(data)=>this.setState({accident: data})}/>
                                </View>
                            </View>
